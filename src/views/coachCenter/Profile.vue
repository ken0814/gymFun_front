...
#section02
  n-form(
    ...
    n-form-item(
      path="contentOfCourses"
      label="教學內容"
    )
      n-checkbox-group(
        v-model:value='form.contentOfCourses'
      )
        n-checkbox(
          value="健體"
          label="健體"
...
const submitForm = async () => {
  ...
  for (const key in form) {
    ...
    } else if (key === 'contentOfCourses') {
      for (let i = 0; i < form[key].length; i++) {
        fd.append('contentOfCourses[]', form[key][i])
      }
    }
...
const init = async () => {
  try {
    ...
    form.contentOfCourses = user.contentOfCourses
    form.time = user.time
